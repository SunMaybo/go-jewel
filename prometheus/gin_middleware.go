...
var reqCnt = &Metric{
	...
	Type:        "counter_vec",
	Args:        []string{"code", "method", "handler", "host","remoteAddr"}}
...
func (p *Prometheus) setMetricsPath(router *gin.RouterGroup) {
...
func (p *Prometheus) setMetricsPathWithAuth(router *gin.RouterGroup, accounts gin.Accounts) {

	if p.listenAddress != "" {
		p.router.GET(p.MetricsPath, gin.BasicAuth(accounts), prometheusHandler())
		p.runServer()
	} else {
		router.GET(p.MetricsPath, gin.BasicAuth(accounts), prometheusHandler())
	}
...
func (p *Prometheus) Use(router *gin.RouterGroup) {
...
func (p *Prometheus) handlerFunc() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		p.reqCnt.WithLabelValues(status, c.Request.Method, c.HandlerName(), c.Request.Host,strings.Split(c.Request.RemoteAddr,":")[0]).Inc()
